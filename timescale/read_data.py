...
def cume_prob(data):
    ...
    for i in range(cume_part_probs.shape[0]):
        cume_probs[i] = np.sum(1 - cume_part_probs[i])/part_probs.shape[1]
...
def recalculate_data(data, target_id, YEAR_STEP = 5000):
    ...
    """
    print("Double check the version of the collision's module")
...
def recalculate_slice(data, target_id):
    """
    Takes in a single snapshot of particles, data, and the key of the target,
    target_id, and returns the probabilities per unit time of the
    data with that target
    ...
    for p in range(data.shape[0]):
        data[p,3] = prob_per_time(data[p,0], data[p,1], data[p,2], target)

    return data[:,3]



def test_recalculate_data(data, target_id, YEAR_STEP = 5000):
    """
    Takes in data and the key of the target, target_id, and returns the
    cume_prob of the data with that target
